...
var ajax_button_modal = function (
  ...
  $.ajax({
    ...
    beforeSend: function () {
      ...
      document.getElementById(modal_id).innerHTML =
        '<center><div class="spinner-grow text-primary m-2" style="width: 3rem; height: 3rem;" role="status"><span class="sr-only">Loading...</span></div></center>';
    },
    success: function (response) {
      if (response != null && success != null) {
        success(response);
...
var PositionAltura = function (classs) {
  ...
  $(classs).each(function () {
    ...
    for (var i = 0; i < children.length; i++) {
      var rowHeight =
        children[i].getElementsByClassName("classAbsolute")[0].clientHeight;
...
var GlobalInitialDatatable = function (element) {
  ...
  $("#" + element + " thead tr")
    .clone(true)
    .appendTo("#" + element + " thead");
  $("#" + element + " thead tr:eq(1) th").each(function (i) {
    if (i < $("#" + element + " thead tr:eq(1) th").toArray().length) {
      var title = $(this).text();
      $(this).html(
        '<input type="text" class="form-control input-sm" placeholder="Buscar en ' +
          title +
          '" />'
      );
...
var changePassword = function () {
  ...
  let PersonalPasswordConfirm = document.getElementById(
  ...
  ajax_(
    ...
    function (response) {
      if (!response.error) {
        ...
        if (document.getElementById("ClienteTipo") == "B2C") {
...
var LineaCreditoTipoCambio = function (Elem) {
  if (Elem.checked) {
    ...
    ajax_(
      "../../models/Pedido/Pedido.Route.php",
      "POST",
      "HTML",
      {
        Action: "lineaCredito",
...
var EmailCatalogo = function (Elem) {
  gtag("event", "conversion", {
    send_to: "AW-1069545026/ixqYCIPh0MIBEMLs__0D",
  });
  ajax_(
    "../../models/Catalogos/Cursos.php",
    "POST",
    "JSON",
    $("#form-catalogo").serialize(),
    function (response) {
      if (!response.error) {
        $(".catalogo").val("");
        templateAlert(
          response.typeError,
          "",
          "En breve iniciará tu descarga",
          "topRight",
          "check-circle"
        );
        var a = document.createElement("a");
        var url =
          "https://fibremex.com/fibra-optica/public/catalogos/Fibremex-Catálogo 2021.pdf";
        a.href = url;
...
function validacionCantidad(Elem) {
  //Solo numeros
...
function validacionCantidad_(Elem, Tamano) {
  //Solo numeros
