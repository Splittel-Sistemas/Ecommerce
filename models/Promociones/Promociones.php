...
class CatalogoPromociones{
  ...
  /**
   * Obtención de información relevante catalogo Promociones
   *
   * @param string $a Foo
   *
   * @return int $b Bar
  ...
  public function get($filter, $order, $return_json){
    try {
      ...
      if (!$this->conn->conexion()->connect_error) {
        $SQLSTATEMENT = "SELECT * FROM menu_promociones ".$filter." ".$order;
        // echo $SQLSTATEMENT;
    ...
    try {
      if (!$this->conn->conexion()->connect_error) {
        ...
        if (!$result['error']) {
          ...
          }elseif($this->Descripcion == 'Promociones'){
            $Email->MailerSubject = "Promociones";
            $this->Correo = ['cursos@fibremex.com.mx'];
            $Email->MailerBody = $EmailTest->cursos($this->Curso);
          }else{
