<?php 
  ...
  }if (!class_exists("PedidoCanjeoTransferencia")) {
    include $_SERVER["DOCUMENT_ROOT"].'/fibra-optica/models/Pedido/B2B/PedidoCanjeoTransferencia.Model.php';
   ...
   * 
   */
  class PedidoCanjeoTransferenciaController{
    protected $conn;
    protected $Tool;
    ...
    public function create(){
      try {
        if (!$this->conn->conexion()->connect_error) {
          ...
          $ResultPedidoController = $PedidoController->ListPedidoB2B_();
      ...
      try {
        ...
        if ($ClienteExiste) {
          ...
          if ($ContactoExiste) {
            ...
            $PedidoB2BModel = new PedidoCanjeoTransferencia();
            ...
            $ResultPedidoB2B = $ResultPedidoB2BModel->CreatePedidoB2BResult;
            ...
            if (!$ResultSalesQuatation['error']) {
               ...
               $LogModel->SetTitle("Pedido B2B Credito");
    ...
    */
    public function ArticulosPedido(){
      try {
        ...
        $ResultDetalleController = $DetalleController->GetDetallePedidoPuntos("WHERE pedidokey = '".$this->Pedido->Key."' AND detalle_activo = 'si' ");
        $items = [];
        ...
        foreach ($ResultDetalleController->records as $Info) {
          $items[] = [
            ...
            "Quantity"          => 1,
            "UnitPrice"         => $Info->ProductoPrecio,
            "Description"       => $Info->ProductoDescripcion,
            "DiscountPercent"   => 0,
            "TaxCode"           => $TaxCode,
