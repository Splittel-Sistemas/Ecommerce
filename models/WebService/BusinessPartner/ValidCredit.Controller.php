 ...
 */
  ...
  class ValidCreditController{
     ...
     * @return int $b Bar
     */ 
    public function GetValidCredit($Importe,$DocCur,$TipoCambio){
      try {
          ...
          if ($ClienteExiste) {
            ...
            if ($ContactoExiste) {
              ...
              $ValidCredit['ValidCredit'] = [
                "CardCode" => $ClienteModel->GetCardCode(),
                "importe" => $Importe,
                "DocCur" => $DocCur,
                "tipoCambio" => $TipoCambio
              ];
              // print_r($UpdateRefDelivery);
              $result = $WebServiceSOAP->ExecuteSoap("ValidCredit", $ValidCredit, false);
              // print_r($result);
